...
void Juego::comprar_bombas(Jugador* jugador){
    ...
    if(jugador->obtener_cant_energia() >= 5){
        ...
        int cantidad_actal_bombas_compradas = jugador->obtener_bombas_compradas();
        ...
        if(cantidad_andycoins >= total_pagar){
            ...
            while(posicion < jugador->obtener_inventario()->obtenerCantidad() && lo_encontro == false){
                ...
                if(jugador->obtener_inventario()->obtenerDato(posicion)->getNombre() == BOMBAS){
                    ...
                    jugador->establecer_bombas_compradas(cantidad_actal_bombas_compradas + cantidad_bombas);
            ...
            if(*jugador->obtener_primer_objetivo() == EXTREMISTA || *jugador->obtener_segundo_objetivo() == EXTREMISTA){
                verificar_objetivo(EXTREMISTA, jugador);
            }
...
Coordenada Juego::cambiar_coordenada(){

    bool cambio_coordenada = false;   
    Coordenada coordenada;          
                
    while(cambio_coordenada == false){

        coordenada.setFila(rand() % mapa->getCantFilas());
        coordenada.setColumna(rand() % mapa->getCantColumnas());
        ...
        if(mapa->getCasillero(coordenada)->getTipo() == CAMINO || mapa->getCasillero(coordenada)->getTipo() == BETUN || mapa->getCasillero(coordenada)->getTipo() == MUELLE){
            ...
            if(mapa->getCasillero(coordenada)->getMaterial() == NULL){

                cambio_coordenada = true;
...
int Juego::obtener_cant_casilleros_disponibles(int cant_casilleros_usados, int cant_aleatoria){
    
    int cant_casilleros_disponibles;
    int cant_total_casilleros_transitables = mapa->obtenerCantDeCasilleros(MUELLE) + mapa->obtenerCantDeCasilleros(BETUN) + mapa->obtenerCantDeCasilleros(CAMINO);
    int total_casilleros = cant_casilleros_usados + cant_aleatoria;
    if(total_casilleros > cant_total_casilleros_transitables){
        if(total_casilleros - cant_total_casilleros_transitables  == 1 && cant_aleatoria == 4){
            cant_casilleros_disponibles = 3;
        }
        else if(total_casilleros - cant_total_casilleros_transitables  == 2 && cant_aleatoria == 4){
            cant_casilleros_disponibles = 2;
        }
        else if(total_casilleros - cant_total_casilleros_transitables  == 1 && cant_aleatoria == 3){
            cant_casilleros_disponibles = 2;
        }
        else if(total_casilleros - cant_total_casilleros_transitables  == 3 && cant_aleatoria == 4){
            cant_casilleros_disponibles = 1;
        }
        else if(total_casilleros - cant_total_casilleros_transitables  == 2 && cant_aleatoria == 3){
            cant_casilleros_disponibles = 1;
        }
        else if(total_casilleros - cant_total_casilleros_transitables  == 1 && cant_aleatoria == 2){
            cant_casilleros_disponibles = 1;
        }
    }
    else{
        cant_casilleros_disponibles = cant_aleatoria;
    }

    return cant_casilleros_disponibles;
}

void Juego::realizar_iteraciones_materiales(Coordenada coordenada, string tipo, int cant_aleatoria, int &cant_casilleros_usados){
    ...
    if(cant_aleatoria != 0){
        int cant_casilleros_disponibles = obtener_cant_casilleros_disponibles(cant_casilleros_usados, cant_aleatoria);
        anulador ++;

        for(int i = 0; i < cant_casilleros_disponibles; i++){
            ...
            if(cant_casilleros_disponibles > 1 && cant_casilleros_usados + 1 != cant_total_casilleros_transitables){
                coordenada = cambiar_coordenada();
            }

            cant_casilleros_usados ++;
...
int Juego::obtener_cant_casilleros_transitables_usados(){

    int cant_casilleros_usados = 0;
    Coordenada coordenada;
    for(int i = 0; i < mapa->getCantFilas(); i++){
        for(int j = 0; j < mapa->getCantColumnas(); j++){
            ...
            if(mapa->getCasillero(coordenada)->getTipo() == CAMINO || mapa->getCasillero(coordenada)->getTipo() == MUELLE || mapa->getCasillero(coordenada)->getTipo() == BETUN){
...
void Juego::lluviaDeRecursos(){
    ...
    int cant_casilleros_usados = obtener_cant_casilleros_transitables_usados();
    int cant_total_casilleros_transitables = mapa->obtenerCantDeCasilleros(MUELLE) + mapa->obtenerCantDeCasilleros(BETUN) + mapa->obtenerCantDeCasilleros(CAMINO);


    while(!se_agrego && cant_casilleros_usados != cant_total_casilleros_transitables){
        ...
        if(mapa->getCasillero(coordenada)->getTipo() == CAMINO || mapa->getCasillero(coordenada)->getTipo() == BETUN || mapa->getCasillero(coordenada)->getTipo() == MUELLE){
            ...
            if(mapa->getCasillero(coordenada)->getMaterial() == NULL){
                // PIEDRA
                if(cant_casilleros_usados < cant_total_casilleros_transitables){
                    cant_aleatoria = (rand() % 1 + 1);
                    realizar_iteraciones_materiales(coordenada, PIEDRA ,cant_aleatoria, cant_casilleros_usados);
                ...
                if(cant_casilleros_usados < cant_total_casilleros_transitables){
                    ...
                    realizar_iteraciones_materiales(coordenada, MADERA ,cant_aleatoria, cant_casilleros_usados);
                ...
                if(cant_casilleros_usados < cant_total_casilleros_transitables){
                    ...
                    realizar_iteraciones_materiales(coordenada, METAL ,cant_aleatoria, cant_casilleros_usados);
                ...
                if(cant_casilleros_usados < cant_total_casilleros_transitables){
                    ...
                    realizar_iteraciones_materiales(coordenada, ANDYCOINS ,cant_aleatoria, cant_casilleros_usados);
                }
                se_agrego = true;
            }
        }
    ...
    if(cant_casilleros_usados == cant_total_casilleros_transitables){
        system("clear");
        cout << "Ya no hay casilleros disponibles para insertar materiales. " << endl;
    }
...
void Juego::verificar_objetivo(string nombre_objetivo, Jugador* jugador){
    ...
    if(nombre_objetivo == COMPRAR_ANDYPOLIS){
        ...
        int cant_bombas_compradas = jugador->obtener_bombas_compradas();

        if(cant_bombas_compradas >= 500){

            cumplir_objetivo(EXTREMISTA, jugador);
        }
    }
}
